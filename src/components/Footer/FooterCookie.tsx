...
export default function FooterCookie() {
    ...
    if (checkCookie) {
        ...
        return (
            <div className={`fixed px-[8px] bottom-[8px] w-full items-center justify-center left-0 transition-all ${scroll < 150 ? '-translate-y-1/2 opacity-0' : 'translate-y-0 opacity-100'}`}>
                <CardPixel className="" title="" size="full">
                    <div className="flex">
                        <div>
                            <Image src="/shiba-logo-tp.png" alt="shiba-icon" width={70} height={70} priority />
                        </div>
                        <div>{t('cookie')}</div>
                    </div>
